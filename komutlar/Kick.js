...
exports.run = async(client, message, args) => {
    ...
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     if(!user) return message.channel.send('<a:Uyar:800986373263720469> |  ``Bir kişi etiketlemelisin.``')
     if(user.id === message.author.id) return message.channel.send('<a:Uyar:800986373263720469> |  ``Kendini Kickleyemezsin``')
     if(user.id === client.user.id) return message.channel.send('<a:Uyar:800986373263720469> |  ``Botu Kickleyemezsin``')
  if(user.id === message.guild.ownerID) return message.channel.send('<a:Uyar:800986373263720469> |   ``Sunucu sahibini Kickleyemessin Zaten Yetkim Yetmez :)``')
    if (!message.guild.member(user).bannable) return message.reply('<a:Uyar:800986373263720469> |   ``Bu kişinin rolü senden üstte veya `Üyeleri yasakla` yetkisine sahip.``');
  ...
  message.guild.members.cache.get(user.id).kick({
    ...
    .setColor('0x36393E')
    .setTitle('Kick')
    .addField('Yetkili', `${message.author.tag}`)  //ahmetosmantr#1453
