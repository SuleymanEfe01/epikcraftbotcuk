...
message.channel.send(new MessageEmbed().setDescription(`${message.author} Reklam filtersini başarıyla \`açıldı.\`.`).setColor('RANDOM').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
}
if (args[0] == 'kapat') {
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} Reklam filtersini başarıyla \`kapatıldı.\`.`).setColor('RANDOM').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
}
};
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['advertisement', 'reklam', 'reklamengel', 'v!reklam-engel'],
  permLevel: 0
};
